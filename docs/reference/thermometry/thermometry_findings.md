...
> step is **superseded** — the current m′=0 engine (`../src/engines/thermometry.py`) reads the cooling pair
...
> **m′=0 RECONCILIATION (2026-06-20).** This audit was first written against the m′=2 *stretched* cooling scheme, whose dark state is |1,+1⟩. The cooling scheme is now settled as **m′=0 clock, config A** — the control↔probe leg-swap was tested and **rejected** (A holds ~3.8×; see `../clock_leg_swap_finding.md`, CLAIMS LS2 [V]) — whose dark state is **|1,−1⟩**. Two consequences propagate below: (i) the park→read transfer is **|1,−1⟩ → |1,0⟩ (Δm=+1)**, not |1,+1⟩→|1,0⟩ (Δm=−1); (ii) the m′=0 clock cooling pair |1,−1⟩/|2,+1⟩ is *itself* first-order B-insensitive (both g_F·m_F=+½, CLAIMS B1), so the readout's field-robustness is now a consistency property of the whole sequence rather than a rescue from field-sensitive cooling. The audited readout results ([O1]–[O4]: the +0.010 pedestal, the 20–60 µs pulse window, the θ-cancellation) depend only on the clock-pair readout physics, not on the dark-state label, and are **unchanged**.
...
- **Required pre-step (park → read):** cooling parks ≈94 % of the population in the dark state |1,−1⟩ (m′=0 clock, config A), not in the clock readout state. A microwave/RF **Δm=+1** carrier π-pulse transfers |1,−1⟩ → |1,0⟩ before readout. It uses the clock-referenced microwave/RF state-prep chain already present, so it is not extra apparatus.
...
- **[V]** The clock pair |1,0⟩↔|2,0⟩ is first-order magnetically insensitive (the readout does not smear under the in-fiber B-noise; quadratic Zeeman only, ~Hz-level at 3 G). With the bare-mirror Δm=0 geometry above, this is why the readout uses |1,0⟩↔|2,0⟩: the m′=0 cooling pair |1,−1⟩↔|2,+1⟩ is itself B-insensitive but is a Δm=+2 pair (the QWP-mirror interferometry path, not the Doppler-sensitive sideband path).
