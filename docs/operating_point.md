...
- Two-photon detuning **δ₂ servoed to the dark resonance**, set-point ≈ +0.1 MHz (dual-end) / ≈ +0.22 MHz (single-ended tagged) — **positive = the compensation of the ≈ −0.2 MHz e3 Stark shift** (this doc's / SSOT convention; §3 uses the same sign). *Cross-doc note:* the master §3, INDEX, and `eit_cooling_tool.py`'s `delta2` field report the **same servo point with the opposite sign** — the raw two-photon detuning ≈ **−0.10 (dual) / −0.19 (single)** under the repo's "(field) − (transition)" convention. Same physical point (δ₂ is servoed, not hardcoded); unifying the two sign conventions repo-wide is a documented open cleanup. Parking at the bare hyperfine still costs the floor; the offset drifts with power/radius and must be servoed, not hardcoded.
