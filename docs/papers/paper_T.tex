% ======================================================================
%  *** HOLD 2026-06-17 — DO NOT SUBMIT AS STANDALONE ***
%  Central result is PRIOR ART: Naber, Torralbo-Campo, Hubert & Spreeuw,
%  Phys. Rev. A 94, 013427 (2016) [arXiv:1605.05230] — same magic pair,
%  same Dm=2 sigma+/sigma- geometry, same destructive-interference null,
%  same "cannot detune away spontaneous emission" conclusion.
%  Scope also narrower than claimed: the m=0 clock transition is Dm=0 and
%  IS coolable. Demoted to P1 motivation. See docs/papers/novelty_findings.md.
%  This source is retained for reuse of its verified core, cited to the above.
% ======================================================================
...
high-fidelity qubit operations, and the coherent transport of atoms in optical tweezers, lattices,
and hollow-core fibres~\cite{eschner2003,leong2020}. The states one most wants to cool are the
\emph{clock} states---the hyperfine-Zeeman sublevels whose splitting is first-order insensitive to
magnetic field, realized by the $m_F=0$ pair or, more generally, by a pair of sublevels with
matched $g_F m_F$. These sublevels store coherence and define the metrological transition, so
cooling them \emph{directly}---rather than cooling an auxiliary cycling state and inheriting its
motion---is the natural goal.

Raman sideband cooling (RSC) is the standard tool~\cite{winelanditano1979,eschner2003}: two laser
fields drive a stimulated two-photon transition that removes one motional quantum, an incoherent
repump recycles the internal state, and iteration pumps the atom toward the dark motional ground
state. RSC is fast and deep, routinely reaching $\nbar\ll1$ on suitable transitions. It is natural
to ask whether it can be applied directly to a field-insensitive clock pair.

We show that, for alkali atoms, it cannot. Imparting a single motional quantum along the trap axis
with a guided or retroreflected beam pair forces the clock-pair Raman to be a $\Dm=2$ transition,
and a $\Dm=2$ two-photon operator is a rank-2 spherical tensor whose matrix element vanishes
identically within a $J=\tfrac12$ ground manifold. The transition is not forbidden outright---it is
recovered through excited-state hyperfine mixing---but only with an amplitude suppressed by the
ratio of the excited-state hyperfine splitting to the detuning. The result is a hard ceiling on the
coherence accumulated per spontaneously scattered photon that, crucially, \emph{cannot be lifted by
detuning}, and that leaves the cooling floor far above the ground state. We compute the obstruction
from first principles (Sec.~\ref{sec:obstruction}), derive the figure-of-merit ceiling and its
detuning independence (Sec.~\ref{sec:fom}), translate it into a cooling floor
(Sec.~\ref{sec:floor}), contrast it with electromagnetically-induced-transparency (EIT) cooling on
the \emph{same} pair (Sec.~\ref{sec:eit}), and show that it is universal across the alkali series,
in fact strongest for the light alkalis (Sec.~\ref{sec:generality}).

% ======================================================================
\section{The clock pair and the $\Dm=2$ requirement}\label{sec:clockpair}
% ----------------------------------------------------------------------
We take $^{87}$Rb as the concrete example; the generalization follows in
Sec.~\ref{sec:generality}. The ground manifold $5S_{1/2}$ ($J=\tfrac12$, $I=\tfrac32$) has
hyperfine levels $F=1,2$; consider the pair
\begin{equation}
g_1=\ket{F{=}1,m_F{=}{-}1},\qquad g_2=\ket{F{=}2,m_F{=}{+}1}.
\end{equation}
Both have $g_F m_F=+\tfrac12$ (with $g_{F=2}=+\tfrac12$, $g_{F=1}=-\tfrac12$), so their splitting
is first-order field-insensitive at \emph{any} field and the pair is immune to the first-order
vector light shift. It is a genuine field-insensitive clock qubit---unlike the stretched pair
$\ket{2,+2}/\ket{2,+1}$, which is field-sensitive and not a clock pair.

A single-axis cooling geometry---a guided beam and its retroreflection, or two beams sharing the
trap axis---delivers the two Raman photons with opposite helicity along the quantization axis, one
$\sigma^+$ and one $\sigma^-$, so the internal state changes by $\Dm=m_{F,2}-m_{F,1}=+2$.
Connecting a field-insensitive alkali clock pair with a single axial two-photon kick is thus,
generically, a $\Dm=2$ operation---the geometric fact whose consequences we now examine.
...
\cite{morigi2000} instead operates near resonance, where the excited-state hyperfine structure is
resolved and dark-state cooling does not rely on a far-detuned $\Dm=2$ Raman amplitude; the
residual spontaneous scattering is built into the steady state rather than appearing as an
...
(Fig.~\ref{fig:floor}). One cannot recover the figure of merit by simply pulling the Raman beams
toward resonance: the off-resonant scattering then diverges and the ratio in Eq.~\eqref{eq:fom} is
lost. EIT achieves near-resonant operation by design, using dark-state interference rather than
large detuning to suppress heating, and recent demonstrations of EIT sideband cooling of alkali
atoms~\cite{chiu2025,xin2024} confirm it as the field-insensitive route to the motional ground
state.
...
% ----------------------------------------------------------------------
A selection rule, not a technical limitation, blocks direct $\Dm=2$ Raman sideband cooling of
alkali clock qubits. The $\Dm=2$ two-photon operator demanded by the single-axis geometry is a
rank-2 tensor that vanishes in the $J=\tfrac12$ ground manifold; the transition survives only
through excited-state hyperfine mixing, with an amplitude $\propto\dhfs/\Delta^2$ that pins the
coherence per scattered photon at $\sim\dhfs/\Gamma$---of order a few, and independent of detuning.
The resulting cooling floor sits far above the motional ground state for every alkali, and below
unity figure of merit for the light alkalis, where the excited-state hyperfine structure is
smallest. Electromagnetically-induced-transparency cooling, operating near resonance, evades the
obstruction and is the field-insensitive route to the ground state. More broadly, the argument
bounds any scheme that drives a $\Dm=2$ transition coherently within a $J=\tfrac12$ manifold by
far-detuned two-photon means; the same rank-2 null constrains $\Dm=2$ two-qubit gates and state
preparation that rely on such transitions.
