...
**[O] Decision OPEN.** True swapped floor ∈ [0.0022 (clean, instant-recycle, zero repump cost) … 0.0204 (full, current repumps untuned)] vs **0.0061 current** — neither "A holds" nor "swap wins" is established. Resolved ONLY by the **deciding run**: swapped legs + repumps **re-pointed & re-optimized for the F=2-dark topology** + δ₂ re-servoed (`clock_combined_solve`, m′=0, single-ended) — does it recover the 3.29× (→ ~0.002–0.003) or stay near 0.0204? Sub-questions: which channel re-pointing must fix (the |2,+2⟩/|2,0⟩ F=2-leak); the F′=3 increment at Dc=80 (unquantified — sub-dominant in the stretched case); magic-B insensitivity survives the swap in-solver (expected — dark pair unchanged — but verify, don't infer from g·m). Prevedelli σ+-forward feasibility is now **secondary** to the repump rebuild. Queued behind the reabsorption-brief. The repump re-pointing is the risky part — rebuild deliberately, do not hack the existing flags. *Retires the leg-swap brief's stretched-scheme numbers and **both** directional claims; the branching primitive is locked here, the deciding floor becomes a CLAIMS entry when the run lands.*
