...
**Also this session, after the v17 consolidation — the pedagogical layer + a figure red-team; no physics conclusion changed (merged PR #9/#10).** A front-to-back guide (8 chapters in [`docs/guide/`](guide/), with its cover folded into the [README](../README.md)) was built over this brief, the figure set embedded, and `src/` subdivided into `engines/` + `tools/`. An independent **cross-chat figure red-team** then ran against the SSOT: it added the **multilevel cloud-floor figure** ([`fig_cloud_floor_multilevel`](../figures/fig_cloud_floor_multilevel.png), now embedded at §8, replacing the older 3-level one); corrected a `fig_knobspace` solver mislabel and a `radial_survival.py` runtime crash; and flagged **two figures for a compute-bound pixel regen** (only the *pixels* lag — the captions and §3/§5 carry the canonical numbers): `fig_retro_flatness` (on-figure ~0.0049; canonical single-tagged **0.0072**, §5 / `operating_point.md` §3 — removed from this doc, §4) and `fig_delta2_landscape` (δ₂=+0.25 in tagged_solver's state-energy sign; canonical **−0.19 field**, §3). Both are tracked in [INDEX §4](../INDEX.md). The one *physics* correction of the round — the Nf=6 **0.0118 hot-cloud retraction** — is the §8 [UPDATE 2026-06-23] banner and is already folded into §8 + INDEX §1b.
...
*All numbers are from the multilevel QuTiP steady-state solver unless noted. Tags: [V] computed/verified in this program, [I] inferred/estimate, [O] open. Key figures are embedded at the relevant sections; the full set lives in [`../figures/`](../figures/), and the front-to-back narrative walkthrough is the [README](../README.md) chapter guide + [`docs/guide/`](guide/).*
...
![Cloud axial floor — a flat-top box collapses the T_r-driven Gaussian rise (post-retraction multilevel union)](../figures/fig_cloud_floor_multilevel.png)

*Post-retraction (multilevel union): the Gaussian cloud floor rises with T_r [I]; a flat-top box collapses it to cooled **0.0072** [V] / uncooled **≥0.021** [O] (the Nf=6 0.0118 is retracted as under-resolved). Canonical numbers + tiers: [INDEX §1b](../INDEX.md) and the §8 [UPDATE 2026-06-23] banner below. The older 3-level dynamic-MC view is in [guide Ch 06](guide/06_cloud_floor_and_deadwall.md).*
